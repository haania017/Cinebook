"""
CineBook v2 – OS Concurrency Engine (per showtime context)
===========================================================
Architecture upgrade from v1:

  v1:  One global mutex + one global semaphore + one global FCFS queue
       → All movies/halls share the same lock; booking Movie A blocks Movie B.

  v2:  A ConcurrencyContext is created per (showtime_id) and lazily cached.
       Each context has its OWN mutex, semaphore, and FCFS worker thread.

       This means:
         • Concurrent bookings for different showtimes never block each other.
         • The Mutex still prevents race conditions on the SAME showtime seat.
         • The Semaphore still enforces hall capacity for that specific show.
         • FCFS ordering is preserved per showtime — the first request in wins.
